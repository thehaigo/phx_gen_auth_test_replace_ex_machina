defmodule Blog.AccountsTest do
  ...
  describe "change_user_registration/2" do
    ...
    test "allows fields to be set" do
      ...
      changeset = Accounts.change_user_registration(%User{}, user)

      assert changeset.valid?
      assert get_change(changeset, :email) == user.email
      assert get_change(changeset, :password) == user.password
  ...
  describe "apply_user_email/3" do
    ...
    test "validates email", %{user: user} do
      {:error, changeset} =
        Accounts.apply_user_email(user, user.password, %{email: "not valid"})
    ...
    test "validates maximum value for email for security", %{user: user} do
      ...
      {:error, changeset} =
        Accounts.apply_user_email(user, user.password, %{email: too_long})
    ...
    test "validates email uniqueness", %{user: user} do
      ...
      {:error, changeset} =
        Accounts.apply_user_email(user, user.password, %{email: email})
